...
function input(item: string, concurrency: 1 | 2 | 3 = 2): CreateTaskInput {
...
test("三任务服务锁：前三项生成、第四项等待、释放后补位", async () => {
  const root = await mkdtemp(join(tmpdir(), "canvas-simple-pool3-"));
  const store = new TaskStore(root); await store.init();
  const a = await store.create(input("a", 3), []); await submitted(store, a.id);
  const b = await store.create(input("b", 3), []); await submitted(store, b.id);
  const c = await store.create(input("c", 3), []); await submitted(store, c.id);
  await assert.rejects(() => store.create(input("d", 3), []), /位置已占用/);
  await store.transition(b.id, "cancelled", "user");
  const d = await store.create(input("d", 3), []); assert.ok(d.id);
  assert.equal(store.list().filter((task) => !["completed", "failed", "cancelled"].includes(task.status)).length, 3);
});

test("并发合同接受一至三并发，拒绝四并发、多附件和已有会话", () => {
  ...
  assert.equal(parseCreateTaskInput({ ...valid, simpleRender: { ...valid.simpleRender, concurrency: 3 } }).simpleRender?.concurrency, 3);
  assert.throws(() => parseCreateTaskInput({ ...valid, simpleRender: { ...valid.simpleRender, concurrency: 4 } }));
  assert.throws(() => parseCreateTaskInput({ ...valid, simpleRender: { ...valid.simpleRender, concurrency: 0 } }));
