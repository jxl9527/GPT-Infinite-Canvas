...
type ToolIconName="select"|"pan"|"text"|"arrow"|"freehand"|"rectangle"|"undo"|"redo"|"fit"|"arrange";
function ToolIcon({name}:{name:ToolIconName}){
  const common={width:20,height:20,viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:1.8,strokeLinecap:"round" as const,strokeLinejoin:"round" as const,"aria-hidden":true};
  if(name==="select")return <svg {...common}><path d="M6 3l6 15 2.2-5.8L20 10z"/></svg>;
  if(name==="pan")return <svg {...common}><path d="M12 3v10M8 6v7M16 7v6M5 13v3a5 5 0 005 5h4a5 5 0 005-5v-3a2 2 0 00-2-2h-1V8a1.7 1.7 0 00-3.4 0v3"/></svg>;
  if(name==="text")return <svg {...common}><path d="M5 6h14M12 6v12"/></svg>;
  if(name==="arrow")return <svg {...common}><path d="M5 19L19 5M11 5h8v8"/></svg>;
  if(name==="freehand")return <svg {...common}><path d="M4 20l4-1L20 7l-3-3L5 16z"/></svg>;
  if(name==="rectangle")return <svg {...common}><rect x="4" y="6" width="16" height="12" rx="1"/></svg>;
  if(name==="undo")return <svg {...common}><path d="M8 8h8a5 5 0 010 10H9M8 8l3-3M8 8l3 3"/></svg>;
  if(name==="redo")return <svg {...common}><path d="M16 8H8a5 5 0 000 10h7M16 8l-3-3M16 8l-3 3"/></svg>;
  if(name==="fit")return <svg {...common}><path d="M4 9V4h5M15 4h5v5M20 15v5h-5M9 20H4v-5"/></svg>;
  return <svg {...common}><path d="M4 7h16M4 12h9M4 17h16M16 10l3 2-3 2"/></svg>;
}
...
export function BasicCanvas({doc,update,undo,canUndo,canRedo,hidden,onImport,onPreview,onCompare,onEditText,children}:{doc:CanvasProjectDocument;update:Update;undo(redo?:boolean):void;canUndo:boolean;canRedo:boolean;hidden:boolean;onImport(files:File[]):void;onPreview(image:{assetId:string;name:string;versionId:string}):void;onCompare(versionId:string):void;onEditText(id:string):void;children?:ReactNode}){
  ...
  const finish=(source:"up"|"blur"="up")=>{
  ...
  useEffect(()=>()=>{if(compareTimer.current)window.clearTimeout(compareTimer.current);},[]);
  useEffect(()=>{
    ...
    const up=()=>finishRef.current("up");const blur=()=>{finishRef.current("blur");spaceRef.current=false;setSpace(false);};
  ...
  const fit=()=>{const objects=basicObjects(doc);if(!objects.length)return;const x=Math.min(...objects.map(o=>o.x)),y=Math.min(...objects.map(o=>o.y));const w=Math.max(...objects.map(o=>o.x+o.width))-x,h=Math.max(...objects.map(o=>o.y+o.height+32))-y;const left=78,right=20,top=16,bottom=56;const availW=Math.max(120,size.width-left-right),availH=Math.max(120,size.height-top-bottom);const scale=Math.max(.03,Math.min(1,availW/Math.max(1,w),availH/Math.max(1,h)));update(d=>{d.canvas.viewport={scale,x:left+(availW-w*scale)/2-x*scale,y:top+(availH-h*scale)/2-y*scale};return d;});};
  ...
  return <div ref={host} className="sg-stage" tabIndex={0} aria-label="图片画布，空白处拖框选择，中键或空格拖动平移，滚轮缩放" data-basic-canvas="true" data-pan={gesture?.kind==="pan"||undefined} style={{cursor:gesture?.kind==="pan"?"grabbing":space||tool==="pan"?"grab":tool!=="select"?"crosshair":"default"}}
    ...
    <div className="sg-canvas-tools" role="toolbar" aria-label="画布基础工具">
      <div className="sg-tool-group">
        {([["select","选择","V"],["pan","平移","H"]] as const).map(([t,label,key])=><button key={t} type="button" title={`${label} ${key}`} aria-label={label} aria-pressed={tool===t} onClick={()=>{setTool(t);host.current?.focus();}}><ToolIcon name={t}/></button>)}
      </div>
      <div className="sg-tool-group">
        {([["text","文字","T"],["arrow","箭头","A"],["freehand","画笔","P"],["rectangle","矩形","R"]] as const).map(([t,label,key])=><button key={t} type="button" title={`${label} ${key}`} aria-label={label} aria-pressed={tool===t} onClick={()=>{setTool(t);host.current?.focus();}}><ToolIcon name={t}/></button>)}
        <input type="color" aria-label="批注颜色" title="批注颜色" value={color} onChange={e=>{setColor(e.target.value);update(d=>{for(const n of d.canvas.nodes)if(ids.includes(n.id)&&n.payload.annotation&&!n.locked)(n.payload.annotation as AnnotationState).color=e.target.value;return d;},true);}}/>
      </div>
      <div className="sg-tool-group">
        <button type="button" title="撤销 Ctrl+Z" aria-label="撤销" disabled={!canUndo} onClick={()=>undo()}><ToolIcon name="undo"/></button><button type="button" title="重做 Ctrl+Y" aria-label="重做" disabled={!canRedo} onClick={()=>undo(true)}><ToolIcon name="redo"/></button><button type="button" title="适合画面 F" aria-label="适合画面" onClick={fit}><ToolIcon name="fit"/></button>
      </div>
      <div className="sg-tool-group"><details><summary title="整理 / 操作" aria-label="整理与操作"><ToolIcon name="arrange"/></summary><div>{([['horizontal','横向排列'],['vertical','纵向排列'],['grid','自动排版']] as const).map(([d,label])=><button key={d} onClick={()=>update(doc=>arrangeBasicObjects(doc,ids,d),true)}>{label}</button>)}<button onClick={()=>select(objects.map(o=>o.id))}>全选</button><button onClick={()=>select([])}>取消选择</button><button disabled={!ids.length} onClick={()=>update(d=>deleteBasicObjects(d,ids),true)}>删除所选</button><button disabled={!ids.length} onClick={()=>update(d=>{const nodes=d.canvas.nodes.filter(n=>ids.includes(n.id));const lock=!nodes.every(n=>n.locked);nodes.forEach(n=>n.locked=lock);return d;},true)}>锁定 / 解锁</button></div></details></div>
    </div>
    {!objects.length&&<div className="sg-canvas-empty"><h2>把图片放在这里</h2><p>拖入图片，或使用顶部导入按钮。也支持 Ctrl+V 粘贴图片。</p></div>}
    <Stage ref={stage} width={size.width} height={size.height} x={viewport.x} y={viewport.y} scaleX={viewport.scale} scaleY={viewport.scale} onMouseDown={begin} onWheel={e=>{e.evt.preventDefault();if(!gestureRef.current)zoom(viewport.scale*(e.evt.deltaY>0?.9:1/.9),stage.current!.getPointerPosition()!);}} onDblClick={e=>{if(compareTimer.current){window.clearTimeout(compareTimer.current);compareTimer.current=null;}let target:Konva.Node|null=e.target;while(target&&!target.getAttr("objectId"))target=target.getParent();const id=target?.getAttr("objectId");const n=doc.canvas.nodes.find(n=>n.id===id);if(n?.type==="image"){const v=doc.versions.find(v=>v.id===n.payload.imageVersionId);if(v)onPreview({assetId:v.assetId,name:String(n.payload.name),versionId:v.id});}else if(n?.type==="text")setEditor({id:n.id,text:(n.payload.annotation as AnnotationState&{text:string}).text,kind:"annotation"});else if(id?.startsWith("text_card_"))onEditText(id);}}>
