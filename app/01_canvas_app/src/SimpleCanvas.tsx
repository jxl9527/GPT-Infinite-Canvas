  ...
  return <section className="sg-canvas-page" hidden={hidden}>
    ...
    <div className={`sg-canvas-layout ${(selected.length || active || settings || selectedText) ? "sg-has-panel" : ""}`}>
      ...
      {(selected.length > 0 || active || settings || selectedText) && <aside className="sg-render-panel" aria-label="渲染面板">
        {settings ? <><div className="sg-panel-title"><h2>画布设置</h2><button onClick={() => setSettings(false)}>关闭</button></div><label>同时生成<select value={prefs!.concurrency} disabled={active} onChange={(e) => updatePreferences({ concurrency: Number(e.target.value) as 1|2|3 })}><option value={1}>1 个任务</option><option value={2}>2 个任务</option>{concurrencyLimit >= 3 && <option value={3}>3 个任务</option>}</select></label><label>每张图生成<select value={copies} disabled={active} onChange={(e) => updatePreferences({ copiesPerImage: Number(e.target.value) as 1|2|3 })}><option value={1}>1 个版本</option><option value={2}>2 个版本</option><option value={3}>3 个版本</option></select></label><p className="sg-small">每张图片使用独立 ChatGPT 网页；提交依次进行，生成时间可以重叠。版本数在“开始渲染”时展开。</p><hr /><h3>导出所选图片</h3><label>保存文件夹<input value={exportPath} onChange={(e) => setExportPath(e.target.value)} placeholder="选择文件夹或粘贴完整路径" /></label><button onClick={() => void run(async () => { const path = await selectProjectFolder(); if (path) setExportPath(path); })}>选择文件夹</button><button className="sg-primary" disabled={!selected.length || !exportPath || exporting} onClick={() => void run(doExport)}>{exporting ? "导出中…" : `导出 ${selected.length} 张`}</button><hr /><details><summary>连接与兼容</summary><p>浏览器扩展：{connection || "未连接"}</p><button onClick={() => void run(async () => { await copyPromptText(await getCurrentBridgeToken()); model.setNotice("连接令牌已复制，请粘贴到扩展设置"); })}>复制本地连接令牌</button><button onClick={() => folderInput.current?.click()}>导入文件夹</button><button onClick={() => void run(openLegacy)}>打开旧版兼容工作台</button></details></>
