...
import { exportSelectedImages, getCurrentBridgeToken, selectProjectFolder } from "./bridge-client";
...
export function SimpleCanvas({ projectId, projectName, onGallery, onProjects, hidden }: {
  ...
  return <section className="sg-canvas-page" hidden={hidden}>
    ...
    <div className={`sg-canvas-layout ${(selected.length || active || settings || selectedText) ? "sg-has-panel" : ""}`}>
      <BasicCanvas doc={doc} update={model.update} undo={model.undo} canUndo={model.canUndo} canRedo={model.canRedo} hidden={hidden} onImport={(files) => void run(() => importFiles(files))} onPreview={(assetId,name) => setPreview({assetId,name})} onEditText={setEditText}>
