...
export function wrapPromptForAction(action: WorkflowAction, prompt: string): string {
  ...
  const instruction = action === "prompt"
    ? "本轮只生成或优化提示词，不生成图片。请保留用户意图，补全结构保护、空间位置、光影逻辑和禁止项，并把可直接使用的成稿放在【最终生成提示词】下。写完最后一条禁止约束后立即结束回复，不得追加提问、征询、总结、评价、邀约或下一步建议。"
